...
def calculate_stats(prices, index, window_size=25):
    ...
    #calculates mean and standard deviation on the local window
    try:
        local_mean = np.mean(local_prices)
        local_std = np.std(local_prices)
        #return a ditionary with stats
...
def clean_group(prices):
    ...
    for index in range(len(prices)):
        #calculate stats for the local window around this point excluding the current point
        local_stats = calculate_stats(prices, index, window_size=10)
        
        #as the maximum a chemsitry price can be is 5000, if the value is above this, give the point a value of 5000
        current_price = prices[index]
        if current_price > 5000:
            cleaned_prices[index] = 5000
        else:
            #check id the current point is an anomaly
            if (current_price < (local_stats['Mean'] - 2 * local_stats['Standard deviation']) or 
                ...
                #replace the point with average of the points adjacent (same logic as before)
                if index == 0:
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        #plot the original data (filtering to only graph anchor data)
        for name, group in df.groupby('Item_name'):
            if name == "Anchor":
                plt.plot(group['Date_time'], group['Price'], label='Original ' + name, color='blue')
        
        #plot the cleaned data (filtering to only graph anchor data)
