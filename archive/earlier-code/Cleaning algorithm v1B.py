...
def clean_group(df):
    #create a list of the prices in the dataframe
    prices = df["Price"].tolist()

    standard_deviation = np.std(prices)#work out the standard deviation
    mean = np.mean(prices)#work out the mean of the data
    for index in range(len(prices)-1):
        if prices[index] < (mean - 2*standard_deviation) or prices[index] > (mean + 2*standard_deviation):#iterates through the data and checks if the price is an anomaly
            if index == 0:#check if it is the first price point
                prices[index] = (prices[index+1] + prices[index+2])/22#reassigning the anomaly the average of the points after it
            elif index == len(prices)-1:#checks if it is the last price point
                prices[index] = (prices[index-1] + prices[index-2])/22#reassigning the anomaly the average of the points before it
            else:
                 prices[index] = (prices[index + 1] + prices[index -1])/2#reassigning the anomaly the average of the points either side

    filtered_prices_dataframe = pd.DataFrame(prices)#convert prices back to a dataframe
    df["Price"] = filtered_prices_dataframe#assigns the price column in the dataframe the new cleaned values
    return df    
...
if __name__ == "__main__":
    ...
    if df is not None:
        ...
        cleaned_df = clean_group(df.copy())
