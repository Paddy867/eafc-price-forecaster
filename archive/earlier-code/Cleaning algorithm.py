...
def calculate_stats(group, index, window_size=10):
    if index - window_size < 0:
        start = 0
    else:
        start = index - window_size

    if index + window_size + 1 > len(group):
        end = len(group)
    else:
        end = index + window_size

    prices = group["Price"].tolist()
    local_prices = prices[start:index] + prices[index+1:end]
    local_mean = np.mean(local_prices)
    local_standard_deviation = np.std(local_prices)
    local_stats = {"Mean": local_mean, "Standard deviation": local_standard_deviation, "Prices": local_prices}
    return local_stats

def clean_group(group_stats, prices):
    for index in range(len(prices)):
        # Check if price is an outlier (beyond 2 standard deviations)
        if (prices[index] < (group_stats['Mean'] - 2 * group_stats['Standard deviation']) or 
            prices[index] > (group_stats['Mean'] + 2 * group_stats['Standard deviation'])):
            ...
            if 1000 < prices[index] < 10000:
                prices[index] = 100 * round(prices[index] / 100)
            elif 0 < prices[index] < 1000:
                prices[index] = 50 * round(prices[index] / 50)
...
def clean_all_data(df):
    ...
    for name, group in df.groupby("Item_name"):
        ...
        cleaned_prices = clean_group(group_stats, group_stats["Prices"])
        group["Price"] = cleaned_prices
        cleaned_data.append(group)
        

    cleaned_dataframe = pd.DataFrame(cleaned_data[0])
