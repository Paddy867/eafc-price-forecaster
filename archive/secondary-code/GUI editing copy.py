...
def Show_graphing_page():
    ...
    if cleaned_df is not None:
        # Debug: Check the type of 'Date_time' before conversion
        print("cleaned_df['Date_time'] dtype:", cleaned_df['Date_time'].dtype)
        print("Sample data:", cleaned_df['Date_time'].head())
        
        # Rename the columns and ensure 'Date' is datetime64[ns]
        market_prices_df = pd.DataFrame({
            "Name": cleaned_df['Item_name'],
            "Date": pd.to_datetime(cleaned_df['Date_time'], errors='coerce'),  # Coerce invalid dates to NaT
            "Price": cleaned_df['Price']
        })
        
        # Debug: Verify the 'Date' column dtype
        print("market_prices_df['Date'] dtype:", market_prices_df['Date'].dtype)
        
        # Set up the page name, title, layout, and sidebar
        st.set_page_config(page_title="Price Data Visualizer", layout="wide")
        st.title("Historical Market Prices")
        st.sidebar.header("Graphs Settings")
        
        # Sort the DataFrame to find the earliest and latest datetime values
        sorted_df = market_prices_df.sort_values(by='Date')
        earliest_date = sorted_df.iloc[0]['Date'].date()
        latest_date = sorted_df.iloc[-1]['Date'].date()
        ...
        # Extract start and end dates from date_range and convert to datetime64
        start_date = pd.to_datetime(date_range[0])
        end_date = pd.to_datetime(date_range[-1])
        
        # Set up the multiselect in the sidebar for item selection
        items_to_select = market_prices_df["Name"].unique().tolist()
        currently_selected_items = st.sidebar.multiselect(
            "Select Items to Graph",
            options=items_to_select,
            default=[items_to_select[0]]
        )
        
        # Filter the DataFrame by selected items and date range
        filtered_market_prices = market_prices_df[
            (market_prices_df["Name"].isin(currently_selected_items)) &
            (market_prices_df["Date"] >= start_date) &
            (market_prices_df["Date"] <= end_date)
        ]
        
        # Check if the filtered DataFrame is empty and then graph the data
        if not filtered_market_prices.empty:
            fig = px.line(
                ...
                y="Price",
                color="Name",
                title="Historical Market Prices"
            )
            fig.update_layout(
                xaxis_title="Date",
                yaxis_title="Price",
                legend_title="Items Graphed",
                hovermode="x unified"
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("No data available for the selected items and date range.")
    else:
...
# To run: streamlit run "C:\Users\paddy\OneDrive - Lancing College\NEA\Code\v4\GUI editing copy.py"